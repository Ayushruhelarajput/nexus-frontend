...
const AI_PERSONALITIES = {
  ...
  CREATIVE: {
    name: "Nexus(Creative)",
    prompt:
      "You are Nexus, a highly creative and imaginative AI assistant. Explore novel ideas and think outside the box.",
    icon: <BoltIcon className="h-4 w-4 mr-1" />,
  },
  TECHNICAL: {
    name: "Nexus (Technical)",
    prompt:
      "You are Nexus, a precise and logical AI assistant. Focus on factual accuracy and technical details.",
...
function App() {
        ...
        if (
          ...
          setBotTypingText((prev) => {
            if (wordIndex < words.length) {
              const newText =
                prev + (wordIndex > 0 ? " " : "") + words[wordIndex];
              wordIndex++;
              return newText;
            } else {
              clearInterval(typingIntervalRef.current);
              ...
              if (!botReplyAddedRef.current) {
                setChatLog((prevLog) => [
                  ...prevLog,
                  { sender: "bot", text: fullText, timestamp: new Date() },
                ]);
  ...
  const stopTyping = useCallback(() => {
    ...
    if (botTypingText.trim() !== "" && !botReplyAddedRef.current) {
      setChatLog((prev) => [
        ...prev,
        { sender: "bot", text: botTypingText, timestamp: new Date() },
      ]);
      botReplyAddedRef.current = true;
    }
  ...
  // 🔥 MAIN FIX: use Flask + Groq backend instead of direct Gemini call
  const sendMessage = useCallback(
    async (predefinedMessage = null) => {
      const messageToSend =
        predefinedMessage !== null ? predefinedMessage : message.trim();
      ...
      if (selectedFile) {
        displayMessage = messageToSend
          ? `${messageToSend} (File: ${selectedFile.name})`
          : `File: ${selectedFile.name}`;
      }

      // Prepare updated chat log (including current message)
      const newEntry = {
        sender: "user",
        text: displayMessage,
        timestamp: new Date(),
      };
      const updatedChatLog = [...chatLog, newEntry];
      ...
      try {
        ...
        // --- "Who created you" logic stays the same ---
        const creatorQuestions = [
          ...
          "who built this webapp",
          "who developed this webapp",
          "this webapp created by",
          "who created this ai",
          "who made this ai",
        ];

        const isCreatorQuestion = creatorQuestions.some((phrase) =>
        ...
        // 🧠 Optional: personality prompt (simple prefix)
        const systemInstruction =
          AI_PERSONALITIES[selectedPersonality].prompt +
          (Object.keys(userMemory).length > 0
            ? `\nUser's persistent memory: ${JSON.stringify(userMemory)}.`
            : "");

        const enrichedLastUserMessage =
          systemInstruction + "\n\nUser: " + messageToSend;

        // For backend, we just send sender + text
        const messagesForBackend = updatedChatLog.map((msg, index) => {
          if (index === updatedChatLog.length - 1) {
            // Last user message → attach system instruction
            return { sender: "user", text: enrichedLastUserMessage };
          }
        ...
        let replyText =
          result.reply || "Sorry, I couldn't get a response from Nexus.";
        ...
        const nameMatch = messageToSend.match(
        ...
        if (error.name !== "AbortError") {
          let errorMsg = `Error connecting to Nexus: ${
          ...
          setChatLog((prev) => [
            ...prev,
            { sender: "bot", text: errorMsg, timestamp: new Date() },
          ]);
        }
      } finally {
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10">
        ...
        {showPersonalityDropdown && (
          <div
            className={`absolute left-0 mt-2 w-48 rounded-lg shadow-lg z-20
            ${
            ...
            {Object.entries(AI_PERSONALITIES).map(([key, value]) => (
              <button
                ...
                className={`flex items-center w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600
                  ${
                    selectedPersonality === key ? "font-bold" : ""
                  } rounded-md`}
              >
                {value.icon}
                {value.name}
      ...
      <div
        ...
        className={`w-full max-w-3xl flex-1 overflow-y-auto rounded-3xl shadow-inset-lg p-4 sm:p-6 mb-4 sm:mb-6 scrollbar-hide
          ...
          ${
            darkMode
              ? "border-t-purple-700/50 border-l-blue-700/50"
              : "border-t-blue-300/50 border-l-purple-300/50"
          }
      ...
      >
        ...
        {chatLog.map((msg, index) => (
          ...
          >
            <span
              className={`text-xs font-medium mb-1 opacity-70 flex items-center ${
                msg.sender === "user"
                  ? darkMode
                    ? "text-blue-300"
                    : "text-blue-600"
      ...
      {/* Input Area */}
      <div
        className={`w-full max-w-3xl flex items-center gap-2 sm:gap-3 p-2 rounded-xl shadow-xl backdrop-blur-lg
        ${
          darkMode
            ? "bg-gray-800/70 border border-purple-700/50"
            : "bg-white/80 border border-blue-300/50"
        }
      ...
      >
        ...
        <button
          ...
          disabled={
            (!message.trim() && !selectedFile && !loading) || !isAuthReady
          }
      ...
      {selectedFile && (
        <div
          className={`flex items-center justify-center space-x-2 p-2 rounded-lg text-xs sm:text-sm mt-2 mb-4
            ${
              darkMode
